...
parser.add_argument('--video_dir', dest='video_dir',
                    help='the directory of videos', default="")
...
def check_input():
    ...
    # for video directory
    if len(args.video_dir):
        if os.path.isdir(args.video_dir):
            videofiles = os.listdir(args.video_dir)
            return 'video', videofiles
        else:
            raise IOError('Error: --video_dir must refer to a directory include video files, not video.')
...
if __name__ == "__main__":
    ...
    for input_name in input_sources:
        ...
        save_path = os.path.join(args.outputpath, save_name)
        if os.path.isfile(save_path):
            print(f"{save_path} exist, jump!")
            continue
        ...
        if args.save_video and mode != 'image':
            ...
            writer = DataWriter(cfg, args, save_video=False, queueSize=queueSize, save_file_name=save_name).start()
